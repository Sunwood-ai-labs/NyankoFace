...
const ACTIONS = new Set(['cancel', 'rerun', 'approve', 'rollback']);

export async function POST(
  request: NextRequest,
  { params }: {
    params: Promise<{ owner: string; repo: string; run: string; action: string }>;
  },
) {
  const { owner, repo, run, action } = await params;
  const denied = await canControlRepository(request, owner, repo);
  if (denied) return denied;
  if (!/^\d+$/.test(run) || !ACTIONS.has(action)) {
    return NextResponse.json({ error: 'Unsupported pipeline action.' }, { status: 422 });
  }
  ...
  const response = await fetch(
    `${RUNNER_API}/pipelines/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/runs/${run}/${action}`,
      ...
      return NextResponse.json(
        {
          error: nativeResponse.status === 404
            ...
            : 'Forgejo could not rerun this workflow.',
