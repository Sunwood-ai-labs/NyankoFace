...
function stripMarkdownCode(value: string): string {
  ...
  let pairingFenced = false;
  let pairingFenceCharacter = '';
  let pairingFenceLength = 0;
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
    ...
    while (removedContainer) {
      ...
      const listItem = !pairingFenced && content.match(/^(\s{0,3}(?:[-+*]|\d{1,9}[.)]))([ \t]+)/);
      if (listItem) {
    ...
    if (pairingFenced && pairingFenceListDepth !== null) {
      const leadingWhitespace = content.match(/^[ \t]*/)?.[0] || '';
      const indentation = leadingWhitespace.replace(/\t/g, '    ').length;
      const requiredIndentation = pairingFenceListIndentation ?? pairingFenceListDepth * 2;
      listContainerDepth = indentation >= requiredIndentation ? pairingFenceListDepth : 0;
    }
    if (
      pairingFenced
      && (
        pairingFenceBlockquoteDepth !== blockquoteDepth
    ...
    if (fence) {
      ...
      if (marker[0] === '\x60' && fence[2].includes('\x60')) continue;
      pairingFenced = true;
      pairingFenceCharacter = marker[0];
      pairingFenceLength = marker.length;
      pairingFenceBlockquoteDepth = blockquoteDepth;
      pairingFenceListDepth = listContainerDepth;
      pairingFenceListIndentation = listItemIndentation;
      continue;
    }

    if (directiveOpeningPattern.test(content)) {
      openDirectiveLines.push(lineIndex);
    } else if (directiveClosingPattern.test(content)) {
