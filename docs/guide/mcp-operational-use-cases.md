...
| UC-03 | Triage repository issues | list_repositories and list_issues return valid bounded lists; get_issue is exercised when the selected repository has an open issue; the combined Forgejo not-found/unauthorized response is recorded as ambiguous, not as established permission or false success |
...
Forgejo returns its combined not-found/unauthorized response, it records
skipped_upstream_ambiguous with the upstream error without claiming that the
token lacks read:issue or treating the call as successful. The fixture tests also
