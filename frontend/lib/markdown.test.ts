...
test('ends an alert before lazy text after a list block', () => {
  const { bodyHtml } = parseReadme([
    '> [!NOTE]',
    '> - # heading',
    'outside',
...
test('keeps invalid fenced-code info as lazy alert content', () => {
  const { bodyHtml } = parseReadme([
    '> [!NOTE]',
    '> paragraph',
    '``` foo ```',
...
test('ends a GitHub alert after a link reference label with a literal backslash', () => {
  const { bodyHtml } = parseReadme([
    '> [!NOTE]',
    '> [\\ ]: /target',
    'outside',
...
test('does not close a list fence on a deindented quoted fence', () => {
  const { bodyHtml } = parseReadme([
    '> [!NOTE]',
    '> - ```',
    '>   list code',
...
test('ends a GitHub alert before quoted type-7 HTML can continue lazily', () => {
  const { bodyHtml } = parseReadme([
    '> [!NOTE]',
    '> <my-widget>',
    'outside',
...
test('ends a GitHub alert before list-nested type-7 HTML can continue lazily', () => {
  const { bodyHtml } = parseReadme([
    '> [!NOTE]',
    '> - <my-widget>',
    'outside',
...
test('rejects over-indented list fence delimiters', () => {
  const { bodyHtml } = parseReadme([
    ':::message',
    '- ```',
    '      ```',
...
test('does not mutate list context while scanning fenced code', () => {
  const { bodyHtml } = parseReadme([
    ':::message',
    '- ```text',
    '  - code-like list item',
  ...
  assert.match(bodyHtml, /data-markdown-block="zenn-message"/);
  assert.match(bodyHtml, /<summary[^>]*>Nested<\/summary>/);
  assert.match(bodyHtml, /<h1[^>]*>After<\/h1>/);
});
